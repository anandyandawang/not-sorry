...
print(texts)
final = next(t for t in texts if t.startswith("I am so "))
words = final.split(" ")
assert "sorry" not in final.lower(), final
assert len(words) == 9, final
assert words[3].islower() and words[4].endswith(",") and words[4][0].isupper() and words[4][1:-1].islower(), final
assert words[5].endswith(".") and words[5][:-1].isupper(), final
assert words[6].startswith("not") and words[6][3:].islower(), final
assert words[7].endswith("ish") and words[7][:-3].islower(), final
assert words[8].islower() and words[8] != "", final
print("OK", final)
PY
